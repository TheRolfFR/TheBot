...
async def cmd_gifmaker(
    bot: discord.Client, message: discord.Message, command: str, args, titre
):
    url_provided = False
    url_gif = None
    ...
    (vrai_width, vrai_height) = _multiline_text_size(
        titre, d, timesBold, FONT_SIZE_CESTVRAI
    )
    (message_width, message_height) = _multiline_text_size(
        sentence, d, times, FONT_SIZE_MESSAGE
    )
    ...
    if width == inside_width:
        inside_width = min(
            int(INSIDE_MAX_WIDTH * max(vrai_width, message_width)), inside_width
        )
    ...
    if not animated:
        d.rectangle(
            [
                (int((width - inside_width) / 2), IMAGE_MARGIN),
                (int((width + inside_width) / 2), IMAGE_MARGIN + inside_height),
            ],
            None,
            OUTLINE_COLOR,
            OUTLINE_WIDTH,
    ...
    if sentence != "":
        ...
        emojiDictionary = {}
        for line in lines:
            (line_width, line_height) = _multiline_text_size(
                line, d, times, FONT_SIZE_MESSAGE
            )
            (coordsY, emojiDictionary) = _drawLine(
                dest,
                d,
    ...
    result = await message.channel.send(
        ...
        file=_image_to_discord_file(images, "cestvrai", duration=duration),
    )
