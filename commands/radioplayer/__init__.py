...
class RadioPlayer:
  ...
  async def playRadio(self, bot, message, command, args):
    ...
    await message.channel.send(
      embed=discord.Embed(
        color=STATUS_COLOR,
        description="Démarrage de la radio " + radioName + " dans le channel " + channel + "...",
      )
    )
    source = radioList.get(radioName)
    print("Playing " + radioName + ": " + source + " in " + channel)

    self.vc.play(discord.FFmpegPCMAudio(source = source ))
  ...
  async def stopRadio(self, bot, message, command, args):
    ...
    if not (self.vc is None):
      ...
      await message.channel.send(embed=discord.Embed(color=ERROR_COLOR, description=f":x: OUCH Le bot n'a pas trouvé le salon de la dernière session :x:"))
