...
async def cmd_jump(bot: discord.Client, message: discord.Message, command: str, args):
    ...
    if probability < JUMP_CHANCE:
        jumpTop[guild_id][author_id] += 1

        await message.channel.send(
            embed=discord.Embed(
                title="Tu as réussi ton jump!",
                description=f"Bravo {message.author.mention} ! Tu passes à **{str(jumpTop[guild_id][author_id])}** !",
                color=CONFIRM_COLOR,
            )
        )
    else:
...
async def cmd_jump_top(
    ...
    topListSorted = list(
        {
            k: v
            for k, v in sorted(jumpTop[guild_id].items(), key=lambda item: item[1])[
                ::-1
            ]
        }.items()
    )
    ...
    resultEmbed = discord.Embed(
        title="Jump leaderboard ("
        + str(len(topListSorted))
        + " joueur"
        + ("s" if len(topListSorted) > 1 else "")
        + ")",
        color=0xD50000,
    )
    ...
    for top in topListSorted[:3]:
        resultEmbed.add_field(
            name="TOP " + str(i + 1),
            value=str(topListSorted[i][1])
            + " pt(s) : "
            + guild.get_member(int(topListSorted[i][0])).display_name,
            inline=False,
        )
        i += 1
    ...
    for i, top_value in enumerate(topListSorted[:JUMP_TOP_DISPLAY]):
        if i > 2:
            ...
            finalString += (
                str(topListSorted[i][1])
                + " pt"
                + ("s" if topListSorted[i][1] > 1 else "")
                + " : "
                + member_name
                + "\n"
            )
    ...
    # your rank
    authorRankList = [
        idx for idx, key in enumerate(topListSorted) if key[0] == author_id
    ]
    ...
    if len(authorRankList) and authorRankList[0] >= JUMP_TOP_DISPLAY:
        finalString += (
            "...\n"
            + str(topListSorted[authorRankList[0]][1])
            + " pt"
            + ("s" if topListSorted[authorRankList[0]][1] > 1 else "")
            + " : "
            + guild.get_member(int(topListSorted[authorRankList[0]][0])).display_name
        )

    if len(finalString):
        resultEmbed.add_field(
            name="Suite top " + str(JUMP_TOP_DISPLAY), value=finalString, inline=False
        )
