...
async def cmd_update_bot(bot: discord.Client, message: discord.Message, command: str, args):
  """
...
":white_check_mark: Access granted :white_check_mark:",
  ...
  with open(REBOOT_MESSAGE_ID_PATH, "w") as f:
    f.writelines([str(result.channel.guild.id) + "\n", str(result.channel.id) + "\n", str(result.id) + "\n"])
    f.flush()
...
async def reboot_successful(bot: discord.Client):
  try:
    ...
    for i in range(5, 0, -1):
      ...
      emb.add_field(name="Reboot successful", value="This message will self-destruct in " + str(i) + " second" + ("s" if i > 1 else ""), inline=False)
