...
class HardlogSettings(guild_settings.GuildSettingGroup):
    ...
    def __init__(self, guild_id: int):
        ...
        super().create_item(
            HARDLOG_KEY_CHANNEL_ID, int, HARDLOG_KEY_CHANNEL_ID_DEFAULT_VALUE
        )
        super().create_item(
            HARDLOG_KEY_ENABLED, bool, HARDLOG_KEY_ENABLED_DEFAULT_VALUE
        )
        super().create_item(
            HARDLOG_KEY_ENABLED_EDIT, bool, HARDLOG_KEY_ENABLED_EDIT_DEFAULT_VALUE
        )
        super().create_item(
            HARDLOG_KEY_ENABLED_DELETE, bool, HARDLOG_KEY_ENABLED_DELETE_DEFAULT_VALUE
        )
...
async def cmd_hardlog(
    bot: discord.Client, message: discord.Message, command: str, args: list
):
    """
    ...
    if len(args) == 1 and args[0] in ["here", "enable", "disable"]:
        ...
        if cmd == "here":
            guild_hardlog_settings.items[
                HARDLOG_KEY_CHANNEL_ID
        ...
        len(args) == 2
        and args[0] in ["enable", "disable"]
        and args[1] in ["edit", "delete"]
    ):
        # get guild id
        guild_id = message.channel.guild.id
...
async def cmd_hardlog_update(
    bot: discord.Client,
    message_original: discord.Message,
    message_edite: any,
    edit=False,
    delete=False,
):
    ...
    if (
        delete == True
        and not guild_hardlog_settings.items[HARDLOG_KEY_ENABLED_DELETE].value
    ):
        return

    if (
        edit == True
        and not guild_hardlog_settings.items[HARDLOG_KEY_ENABLED_EDIT].value
    ):
        return
