...
def _drawLine(img: Image.Image, d: ImageDraw, offsetX: int, offsetY: int, line: str, font: ImageFont, fontSize, emojiDict: dict, fill=None):
  # find emojis
  emojis = _p.findall(line)
...
async def cmd_powered(bot: discord.Client, message: discord.Message, command: str, args):
  """Usage :
...
Génère logo powered by de l'utbm
  ...
  for (text, props) in sets:
    ...
    # draw text
    _drawLine(txt, d, coordsX, coordsY, text, f, fontSize, emojiDictionary, fill=props['c'])
