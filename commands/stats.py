...
def _dict_to_csv_discord_file(dict, filename):
  ...
  return discord.File(writer_file, filename=f"{filename} { datetime.now().strftime('%Y-%m-%d %Hh%Mm%Ss') }.csv")

async def cmd_stats(bot: discord.Client, message: discord.Message, command: str, args: any):
  ...
  # add wait message
  waitMessage = await message.channel.send(embed=discord.Embed(title="Création des stats en cours...", description="Merci de patienter...", color=HELP_COLOR))

  # create result embed
  resultEmbed = discord.Embed(title=f"Stats du serveur {guild.name}", value="Statistiques du serveur au " + datetime.now().strftime("%d/%m/%Y"), color=HELP_COLOR)
  ...
  # members count
  resultEmbed.add_field(name="Nombre de membres", value=str(guild.member_count), inline=False)
  ...
  # add roles
  resultEmbed.add_field(name="Roles", value="​" + ", ".join(roles), inline=False) # the "" \u200 is a zero width space to avoid empty value
  # add ranks
  resultEmbed.add_field(name="Ranks", value="​" + ", ".join(ranks), inline=False) # the "" \u200 is a zero width space to avoid empty value
  ...
  # send final message
  await message.channel.send(embed=resultEmbed, file=_dict_to_csv_discord_file(memberArrivals, 'members'))
...
async def cmd_roles(bot: discord.Client, message: discord.Message, command: str, args: any):
