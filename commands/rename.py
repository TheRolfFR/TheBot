...
def getRenameSettings(message: discord.Message):
    ...
    renameEnableItem = renameSettings.create_item(
        RENAME_SETTING_NAME_SET, RENAME_SETTING_TYPE_SET, RENAME_SETTINGE_DEFAULT_SET
    )

    # get rename original item
    renameOriginalItem = renameSettings.create_item(
        RENAME_SETTING_NAME_ORIGINAL,
        RENAME_SETTING_TYPE_ORIGINAL,
        RENAME_SETTINGE_DEFAULT_ORIGINAL,
    )
...
async def cmd_rename(
    ...
    if arr[INDEX_CHANGE] is not None:
        ...
        try:
            ...
            # else it is
            done = await message.channel.send(
                embed=discord.Embed(
            ...
            await message.author.voice.channel.edit(
                name=value, reason="TheBot renommage par " + message.author.name
            )
    ...
    if arr[INDEX_RESET] is not None:
        ...
        if str(foundChannel.id) in renameOriginalItem.value:
            try:
                ...
                await message.author.voice.channel.edit(
                    name=renameOriginalItem.value[str(foundChannel.id)],
                    reason="TheBot renommage par " + message.author.name,
                )
            except BaseException as e:
                error = await message.channel.send(
                    embed=discord.Embed(
    ...
    if arr[INDEX_ORIGINAL] is not None:
        ...
        changed = await message.channel.send(
            embed=discord.Embed(
                title=":speaker: Rename channel",
                color=CONFIRM_COLOR,
