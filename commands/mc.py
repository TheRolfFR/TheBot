...
async def cmd_mc(bot: discord.Client, message: discord.Message, command: str, args):
    ...
    if subcommand == "colors":
        await message.channel.send(
            file=discord.File(os.path.join(os.getcwd(), "resources", "mc_colors.jpg"))
        )
        return
    ...
    if subcommand == "get":
        ...
        text = text.replace(" ", "+")
        image_url = (
            "https://minecraftskinstealer.com/achievement/"
            + str(random.randint(0, 39))
            + "/Achievement+Get!/"
            + text
        )
    ...
    if subcommand == "lookup":
        ...
        embed = discord.Embed(
            title="Historique des noms pour " + recherche, color=STATUS_COLOR
        )
        for data in name_history:
            if data["changed_to_at"] == 0:
                embed.add_field(
                    name="Premier", value="``" + data["name"] + "``", inline=True
                )
            else:
    ...
    if subcommand == "status":
        ...
        embed = discord.Embed(title="Status des service minecraft", color=STATUS_COLOR)
    ...
    if subcommand == "ping":
        ...
        embed = discord.Embed(title="MSS: " + recherche, color=STATUS_COLOR)
        ...
        if online:
            embed.add_field(
                name="IP", value=str(data["ip"]) + ":" + str(data["port"]), inline=False
            )
            ...
            if data["mods"] is not None:
                embed.add_field(
                    name="Mods",
                    value="Oui (" + str(len(data["mods"]["names"])) + ")",
                    inline=False,
                )
